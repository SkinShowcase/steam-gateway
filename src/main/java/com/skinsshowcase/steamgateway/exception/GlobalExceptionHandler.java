...
public class GlobalExceptionHandler {
    ...
    public ProblemDetail handleInventoryItemNotFound(InventoryItemNotFoundException ex, WebRequest request) {
        ...
        detail.setTitle("Inventory Item Not Found");
        detail.setInstance(URI.create(request.getDescription(false).replace("uri=", "")));
        return detail;
    }

    @ExceptionHandler(ItemsServiceException.class)
    public ProblemDetail handleItemsService(ItemsServiceException ex, WebRequest request) {
        log.warn("Items service error: {}", ex.getMessage());
        var detail = ProblemDetail.forStatusAndDetail(HttpStatus.BAD_GATEWAY, "Items catalog service error");
